...
| `@DIARY_TEXT_K{id}, PAGENUM, MODE, PAGECOUNT` | body。`MODE` ∈ `"デイリー"`（自动日终）/ `"指令"`（命令 406）。**body 必须在 `@` 头行之后的前三行用 `#DIM PAGENUM` / `#DIMS MODE` / `#DIM PAGECOUNT` 声明这些自定义命名参数**，否则 Emuera 在载入时抛 警告Lv2（「本函数内变量未定义」），且读取这些变量会失败。这与 §1 陷阱 #1 是同一条 Emuera 规则 —— 规范范式另见 `006 Luna [ルナ]/ルナチャイルド/M_KOJO_K6_日記.ERB:340-349`。 |
...
**`@M_KOJO_EVENT_K{id}_3(ARG, ARG:1)` —— 就寝。** 与 `_2` 相同的「地图 vs cell」区分。需要同样的守卫。
...
| 3 | おやすみ（就寝） | 1=自室帰宅 / 2=眠そう / 3=睡眠中 / 4=イタズラ中 / 5=衰弱中 / 6=MASTER気絶 | — | **强制 cell 守卫** |
