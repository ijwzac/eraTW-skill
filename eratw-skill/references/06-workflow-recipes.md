...
三个常见口上编写任务的详细走查。SKILL.md §9 有精简版。

## 11. 工作流配方

> **关于这三个配方**：它们是按*规模*排序的，而不只是按主题。
>
> - **11.1** 最重 —— 从一个空目录搭建全新口上。你会生成数十个文件和许多标签。当目标角色还完全没有口上时用它（例如一个 賑やかし 占位桩或一个完全空的目录）。
> - **11.2** 中等 —— 给一个*已经能工作*的口上添加一个*新功能*（一个带自己进度标志的、重量级一次性脚本事件）。你会改动既有文件，但增量很可观：一个新标签、一个状态进度 CFLAG、在另一个文件里挂一个触发钩子。
> - **11.3** 最轻 —— 对单个既有标签 body 做一处*小内容调整*。一个文件、加一个分支、不加新标签或标志。
>
> 如果用户请求含糊，先问清他们指的是哪个规模，再开始搭建。

### 11.1 从零搭建新角色变体

用户请求示例:「我想为 021 Merlin (中文名梅露兰) 写一份口上。她是三妖精之一，性格大方爽朗，喜欢音乐和恶作剧，对%CALLNAME:MASTER%有姐姐式的关怀感。」

(已确认：`021 Merlin [メルラン]/` 目前是一个空的 `HOGE` 占位符，所以我们是从零开始搭建。)
...
├── M_KOJO_K21_イベント.ERB          ; existence + FLAGSETTING + COLOR + UPDATE + ENCOUNTER + ~5 个 EVENT 槽位
├── M_KOJO_K21_日常系コマンド.ERB    ; 至少 ~10 条日常命令
├── M_KOJO_K21_セクハラコマンド.ERB  ; 可以全是 LOCAL=0 空桩
├── M_KOJO_K21_性交系コマンド.ERB    ; 可以全是 LOCAL=0 空桩
├── M_KOJO_K21_愛撫系コマンド.ERB    ; 空桩即可
├── M_KOJO_K21_加虐系コマンド.ERB    ; 空桩即可
├── M_KOJO_K21_道具系コマンド.ERB    ; 空桩即可
├── M_KOJO_K21_派生コマンド.ERB      ; 空桩即可
├── M_KOJO_K21_カウンター.ERB        ; ~5-10 个反击
├── M_KOJO_K21_弾幕勝負.ERB          ; 一个 DANMAKU 标签
├── M_KOJO_K21_刻印取得.ERB          ; 一个 MARKCNG 标签
├── readme.txt                         ; 人设 / 版本 / 授权
└── ライセンステンプレ.txt              ; 授权模板
```
...
;CFLAG:21:眠姦口上有 = 0
;CFLAG:21:扮演口上有 = 0
;CFLAG:21:推倒禁止 = 1                ; 默认拒绝

;### 口上色判定 ###################################
@M_KOJO_COLOR_K21
SETCOLOR 0xC8E6F0                       ; 淡蓝色，契合梅露兰幽灵之翼的色调
...
;==================================================
;301,泡茶
;...其余命令同样的 IF FLAG:時間停止 / ELSEIF TALENT:恋慕 / ... 级联
```
...
(与下方 §11.3 对比：这是一个*新功能*（带自己进度标志的多行演出剧情），不是一处小台词调整。我们要新增一个全新标签 `@K1_NEW_YEAR_AMAZAKE`、一个用于「已经体验过」的新 CFLAG 槽位、以及在 `@SPECIALDAY_EVENT_K1` 里的一个钩子。既有口上其余部分不变。)

**Step 1 — 决定触发位置.**

此事件适合 `@SPECIALDAY_EVENT_K1` (节日/纪念日 hook), 因为它有明确的日期条件。如果灵梦的现有口上已经定义了这个 label, 就在末尾追加; 没有就新建。触发条件: `DAY:2 == 1 && DAY:3 == 1 && CFLAG:MASTER:現在位置 == <博丽神社地点 ID> && ABL:1:親密 >= 6 && CFLAG:1:1101 == 0`.
...
(与 §11.2 对比：这只是给一个既有 body *加一个额外分支*。没有新标签、没有新标志、没有进度状态。补丁小到可以直接以 diff 形式展示。)
...
| 症状 | 可能原因 | 检查 |
|---|---|---|
| 引擎只打印默认旁白 | 存在判定返回 0 或缺失 | `@M_KOJO_K{id}(ARG)` 存在且 `RETURN 1`? |
| 角色声线不对 | RESULTS selector 不匹配 | body 的标签前缀是否与存在判定的 `RESULTS` 匹配? |
| 某条命令无声 | body 里是 `LOCAL = 0` | 若想要输出就设 `LOCAL = 1`. |
| 时间停止场景无声 | 未设 `CFLAG:N:時間停止口上有` | 在 FLAGSETTING 里设 `CFLAG:N:時間停止口上有 = 1`. |
| 睡眠场景无声 | 未设 `CFLAG:N:眠姦口上有` | 设 `CFLAG:N:眠姦口上有 = 1`. |
| 即便同意也拒绝推倒 | `CFLAG:N:推倒禁止 = 1` | 在 FLAGSETTING 里设为 0 (如需要可按关系门控). |
| 双版本冲突 | 两个 variant 目录都处于激活状态 | 删除不需要的 variant, 或用 `[SKIPSTART]/[SKIPEND]` 屏蔽其存在判定. |
| 自定义命令不出现 | `@KOJO_COM_ABLE_K{id}_{Y}` 返回 0 | 追踪该谓词. |
| 日记读不了 | DIARY:N:M 没有转到 1/2/3 | 检查 `@DIARY_BEFORE_CHECK_K{id}`. |
