...
> **状态：已端到端验证通过（2026-07）。** 验证用例＝随 skill 附带的露娜范例口上 `reference-kojo/luna-K6/`——那份口上**有意保留**了完整测试脚手架（测试套件 + `[[MT]]` 标记 + `;@AT` 标签），是本节方法论的活教材，看不懂哪一步就去对照它的实际代码。
...
;=== AUTOTEST 挂钩（临时——交付前删除，见 §11.9）===
;先清空 RESULTS 再读：文件不存在时 LOADTEXT 不会改动 RESULTS，
;会残留上一次的值造成误判。
RESULTS = ""
LOADTEXT "ERB/口上・メッセージ関連/個人口上/<角色目录>/<变体目录>/autotest.txt"
;⚠【绝不能】写成 IF RESULT && STRCOUNT(...)——实测 LOADTEXT【成功】读取后
;  RESULT 也是 0（不是 1、不是行数），那样写会永远短路成「从不触发」。
;  只判断内容里有没有 on。
...
**当前方案（权威定义在 §11.14）：** 开关是口上目录里的 `autotest.txt`，内容 `on` / `off`；钩子读它、测试套件结尾 `SAVETEXT` 写回 `off` 自动关。**与游戏存档完全无关**，启动器也靠这个文件发现「哪些口上带自动测试」。

> **两条被弃用的老方案，别再捡回来：** ①**`DEBUGGERR` 守卫**——开发版把它默认为 0，玩家一开调试就会触发一次；而且把它清零会连带禁用游戏其他调试输出。②**`CFLAG:{id}:1099` 三态待命位**——「已跑过」的记忆存在**每个存档**里，而开关是**全局文件**，粒度对不上：已跑过的存档再也不触发，启动器却仍显示「已启用」，误导用户；而且同存档重跑还得去调试台打字。

> ⚠️ **`autotest.txt` 默认内容必须是 `off`**（对玩家安全：读到 `off` 就什么都不做）。但**交付前仍然必须删挂钩 + 删 `AUTOTEST.ERB` + 删 `autotest.txt`**（§11.9）——只把内容留成 `off` 不算数：那是个玩家可能误改、也会被本 skill 启动器扫到并改写的文件。
...
4. **还原**每个保存的槽位。
5. **自动关掉开关**——结尾 `SAVETEXT "off", "<与钩子同一条 autotest.txt 路径>"`，使其每次启用只跑一次。

```erb
;============================================================
;K{id} 的 AUTOTEST 测试台——临时。交付前删除。
;由 会話 处理器顶端「读 autotest.txt」的挂钩触发（§11.4）。
...
CFLAG:{id}:面識 = sav:5
FLAG:時間停止 = sav:6
SAVETEXT "off", "ERB/口上・メッセージ関連/個人口上/<角色目录>/<变体目录>/autotest.txt"
                           ; 跑完自动关；路径必须【正斜杠】且限 .txt
...
```erb
		;@AT 待手动测试 K{id}_EV1_今日首问候
		SIF K{id}_MT_ON()
		PRINTL [[MT K{id}_EV1_今日首问候]]
		IF TALENT:{id}:恋人
...
- **TID 必须以 `K<id>_` 开头**（例如 `K{id}_…`），这样光看标记就能读出是哪个角色。
...
> **要告诉用户这一点（否则看起来会吓人）：** 手动测试时，看到某段对话前弹出一行像 `[[MT K{id}_EV1_今日首问候]]` 的东西是**正常且预期的**——那是测试标记在尽责，一旦该对话被标为通过它就会被自动移除。
...
1. **交付前解除待命——阻塞项。** 交回口上前：注释掉 会話 顶部那段挂钩、删除 `M_KOJO_K{id}_AUTOTEST.ERB`、并删掉口上目录里的 `autotest.txt`（§11.14 的开关文件）。**只把 `autotest.txt` 的内容留成 `off` 是不够的**——那是个玩家可能误改、也会被本 skill 的启动器扫到并改写的文件。代码和开关文件一起移除才算干净交付。
...
- **用户操作所有游戏内 GUI**：在启动器问「是否启用 autotest」时选启用（写 `autotest.txt` = `on`，§11.14）、读档、点 会話 触发测试套件、选命令、点击过对话。调试控制台（`调试(D)` → `打开调试窗口` → `控制台`）**平时不需要**，只在要手工注入某个状态时才用。
...
> | `start_pipeline.ps1` | **主编排**：选版本 → 清理(杀旧抓取器/归档 cliplog/删 `lazyloading.dat`+`emuera.log`) → `autotest.txt` 开关 → 启游戏 + 隐藏 `clip_tap` → 实时面板 → 关游戏后**崩溃诊断**(见下) + 抽 AUTOTEST + 两次回写提议 |
...
- **AUTOTEST 待命**——开关是口上目录里的 `autotest.txt`（§11.14），默认内容 `off` = 永不触发。要跑测试套件：启动器启动时会列出所有带 `autotest.txt` 的口上并问是否启用，选「全部启用」即可；进游戏读档后点一次 会話 就触发，跑完测试套件会自己把内容写回 `off`。想再跑一次，重开启动器再选一次启用。**不需要在调试台打任何命令，也与存档无关。**
- **`[[MT …]]` 标记是正常的**——手动测试时，某段对话前会出现一行像 `[[MT K{id}_EV1_今日首问候]]` 的东西；那是预期的测试标记、不是 bug，一旦该分支确认通过就会被自动移除。
...
## 11.16 游戏结束后的错误诊断：`cliplog` 与 `emuera.log` 两个日志的分工

关游戏后 `start_pipeline.ps1` 做**两次**错误检查，**两次都只读 `emuera.log`、不读 `cliplog`**（细节以脚本为准，这里讲**为什么**）：

| | 时机 | 判据 | 处置 |
|---|---|---|---|
| **① 编译 / 脚本错误** | autotest / mt **之前** | `emuera.log` 新鲜 **且 <50 行 且**含 `警告Lv\d` / `Emuera停止运行` / `无法解释的行` | 汇报后**直接结束**，跳过 autotest / mt |
| **④ 运行时错误** | autotest / mt **之后** | `emuera.log` 新鲜 **且末 20 行**含 `错误发生` / `错误内容` / `函数调用栈` | 汇报；测试结果照常回写 |

**为什么 ① 要跳过后续**：编译有错时受影响的标签会**静默失效**，此时的测试结果不可信，显示出来只会误导用户以为「测了、没问题」。
**为什么 ④ 放在最后**：游戏跑到一半崩溃时，测试套件往往已经跑完一部分，那些结果仍有价值——先把回写做完，再报崩溃。

> **⚠ 为什么两次都不能用 `cliplog` 判断（踩过的坑）**：曾经 ① 是用「`cliplog` <50 行且含『Emuera停止运行』」判的，结果**漏报**——真实案例里 `cliplog` 有 100 行、且根本没出现「Emuera停止运行」，但引擎其实报了一整串 `警告Lv2`（某口上 IF/ENDIF 不平衡）。原因见下面两个日志的本质区别：**引擎的编译警告和异常堆栈都不走 PRINT→剪贴板那条路**，所以剪贴板流里可能压根没有它们。

下面是这两个日志的本质区别——这也回答了「为什么必须引入 `emuera.log`」：
...
>
> 因此启动器的做法：读游戏根目录的 `emuera.log`，若它**新鲜**（改动在几分钟内——启动时已删过一次，故存在即本次会话）且**末 20 行含崩溃关键词**，就把这 20 行**并进 `cliplog.txt` 末尾**（带醒目分界）并显示给玩家，提示「游戏运行中崩溃，请把 cliplog 发给 AI」。这样 AI 只需读 `cliplog.txt` 一个文件就能拿到崩溃堆栈。（① 编译错误时同理，只是并进去的是 `emuera.log` **全文**——反正它 <50 行。）
...
```
WARN: TIDs in log with no source ;@AT tag: K{id}_COLOR
```
