...
**`@M_KOJO_EVENT_K{id}_3(ARG, ARG:1)` —— 睡眠相关。** 与 `_2` 有相同的「地图 vs 单元格」区分。需要同样的守卫。

> ⚠**触发条件比名字宽得多（实测踩坑）。** 派发源 `ERB\MOVEMENTS\SLEEP.ERB @CHARA_SLEEP`，在**角色的整个睡眠时段内、MASTER 与她同格**时每回合都可能派发一次——**不是只在 `就寝時間` 那一刻**。典型场景：玩家在她 `起床時間` 之前把她吵醒、试图互动（例如带不出房间），引擎发 `ARG=2`（眠そう＝困倦），随后她可能重新入睡。
> **推论：`ARG=2` 的台词绝不能写死时刻**（「都快两点了还不睡」在清晨被吵醒时就会穿帮）——写成与时刻无关的「困倦/被吵醒」反应。露娜初版就踩了这个坑。
...
| EVENT 3 | おやすみ(**睡眠相关**) | **整个睡眠时段内、与 MASTER 同格时每回合都可能触发**（**不是**只在 `CFLAG:就寝時間` 那一刻） | 1=回房睡(`ARG:1` 0普通/1约会中) **2=困倦(被吵醒/被赶出寝室)** 3=睡眠中 5=衰弱 7=… 8=留她休息 9=当场睡着 10=回自宅睡 |
