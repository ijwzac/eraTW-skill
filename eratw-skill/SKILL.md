...
## 0. 先读这一节 —— 作业边界(§0.7 ⛔) / HANDOFF(§0.8) / 备份(§0.9 💾) / 你的角色、模式、输出语言，以及去哪找更多材料
...
**允许写**：`ERB/口上・メッセージ関連/個人口上/<你正在写的角色目录>/` 之内。（唯一的例外是备份目录——见 §0.9 的明确豁免。）
...
**⚠ HANDOFF 的【开头第一段】必须是下面这段「接手自引导」，一字不改地照抄进去。** 它的作用：compact 之后、或换 session / 换 AI 接手时，第一个动作是读 HANDOFF——而这段会立刻把接手者赶去把 skill 知识补齐，不让它靠 compact 摘要里的残片就动手写代码。
...
（把上面这段连同 `>` 引用格式一起抄进 HANDOFF 开头。发布口上给玩家时这段无所谓——HANDOFF 本是开发脚手架、可留可删。）
...
**必备内容——下面每一项都【必须有实质内容】，不能偷工减料。** 一份只列「本轮做了 A、B、C」的改动流水账是**不合格**的：那些从 git log / 读代码就能知道，HANDOFF 要答的是「接手者不看你脑子就无从知晓」的东西。缺了这些，compact 后的 AI 读完照样两眼一抹黑：

- **用户是谁 + 用户的重要指示与偏好**，尤其「铁律」级的——**连同为什么**。（例：「只改本角色口上目录、绝不碰引擎——因为…」。）
- **项目背景与目标**：这个口上为何存在、属于什么更大的任务、相关材料在哪（需求文档 / 参考口上 / skill 路径）。接手者要能据此判断「往哪走」，而不只是「已经做了什么」。
- **当前任务与状态**：在做什么、做到哪、**下一步具体是什么**。
- **待办**：办完就删或收敛成一句结论；别长成考古报告。
- **踩过的坑与教训**：那些「查了半天才发现」的引擎事实，尤其**试过但失败的路子**（最省未来时间——否则接手者原样再踩一遍）。
- **关键路径 + 验证/测试怎么跑**：游戏根、口上目录、启动器/测试流程怎么触发。
- **角色约定**：人设要点、与关联角色（同组伙伴 / 对手）的关系、称呼约定等 compact 会丢的设定。
- **备份约定**（§0.9）：用户选了哪种备份方式、备份目录的完整路径、最近一次备份是什么时候。**这条尤其容易在 compact 后丢**——丢了的后果是接手者以为没备份、要么重复问用户，要么干脆裸奔着改文件。

**别用「本轮」这种只有当下才懂的指代**——compact 后「本轮」指哪轮就没人知道了。每一条都要写成「对一个完全失忆的接手者也成立」的完整陈述。
...
### 0.9 💾 口上备份 —— 动手改之前，先确保用户有退路

> **这是一项你要自己记住、并主动执行的任务。** 用户多半不会提这件事，但口上往往是他们几十上百小时的创作，而你是在**原地覆盖真文件**。

**为什么这条重要**：你的每一次 `Edit` / `Write` 都直接落在用户游戏目录里的真文件上。一次锚点写错、一次整文件覆写、一次「我以为这段是废代码」的删除，就可能抹掉用户几个月的心血；整合包更新、重装、误删同样会端掉整个口上目录。**没有备份时，任何一次不可逆的失误都是永久损失**——而且用户往往很久以后才发现。

**你要做的（按顺序）：**

1. **第一次要实质修改某份口上时，先问用户想怎么备份**，并给出选项：

   | 方案 | 说明 |
   |---|---|
   | **A. 时间戳文件夹副本**（默认） | 把整个口上变体目录**整份复制**到备份目录，目录名带时间戳，例如 `口上备份/露娜切露德_重制_20260718-1432/`。零依赖、谁都看得懂、恢复就是拷回去。 |
   | **B. Git（本地）** | 在备份目录（或口上目录）`git init`，每改完一段就 commit。历史完整、能 diff、能回滚到任意一版。 |
   | **C. Git + 远端（GitHub 等）** | B 再加异地容灾。⚠**推送到远端等于对外发布**：仓库公开还是私有、里面有没有真实姓名之类的隐私，**必须由用户明确拍板**（§0.7 禁止你擅自 `git push`）。 |
   | **D. 用户自己的方案** | 网盘 / 同步盘 / 他们已有的习惯。问清楚就照做。 |
   | **E. 不备份** | 只有用户**明确这么说**，才可以不做。 |

2. **⭐无论用户答没答，默认就按 A 执行。** 不要因为「用户还没回复」就跳过——**只有用户明确说了「不用备份」才可以不做**。这是本节唯一的默认动作，宁可多留一份副本，也不要让用户在事故之后才发现没有退路。

3. **备份时机**：①**首次实质修改之前**（先把一个已知可用的状态固定下来）；②每完成一个阶段性工作之后。别等出了事才想起来——那时已经晚了。

4. **做完要把备份的完整路径告诉用户。** 不说等于没做：用户不知道去哪找，事故时照样恢复不了。

**⚠ 致命细节：备份【绝对不能】放在 `ERB/` 目录树里面。**

引擎加载时会**递归扫描整个 `ERB/`**，任何位置的 `.ERB` 都会被加载（§4「加一个新文件就是全部安装过程」）。把口上目录复制一份留在 `ERB/` 下——**哪怕改了目录名、哪怕名字里写着「备份」「bak」**——引擎都会把**两份一起加载**，于是每个 `@标签` 都被重复定义：轻则你后续的改动「改了完全没反应」（一份定义遮蔽了另一份），重则整个角色行为错乱。**而且它不报编译错误**，属于本 skill 反复强调的那类「静默失败」，极难排查。（§2 的校验脚本里那条 `Duplicate @label` 检查就是为这类问题准备的，但它只查单个目录内部，跨目录的重复它看不到。）

**正确位置**：放在 `ERB/` **之外**。游戏根目录下的 `口上备份/` 是安全的（引擎只扫 `ERB/`）；**更稳妥是放到游戏目录之外**，这样连整合包更新、重装都波及不到。

> **这是 §0.7「写操作只落在口上文件夹里」的一处明确豁免。** 为创建备份，你可以在上述备份目录里写入。豁免范围**仅限于此**，并且是**只增不减**：只往备份目录写新副本，**绝不删除或修改任何已有备份**。

**保留策略**：备份会越攒越多。份数明显变多时可以提醒用户清理，但**不要自己删**——哪一份重要，只有用户知道。
...
> - **💾 动手改之前先确保有备份（§0.9）**——默认就做「整目录 + 时间戳」副本，只有用户明确说不用才跳过；**备份绝不能放进 `ERB/`**（会被引擎一起加载 → 标签重复定义 → 静默出错）。做完把路径告诉用户。
